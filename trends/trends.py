...
def analyze_tweet_sentiment(tweet):
    ...
    temp=0
    trigger=0
    tba=tweet_words(tweet)
    l=len(tba)
    for i in range(l):
        if get_word_sentiment(tba[i]) is not None:
            trigger=trigger+1
            temp=temp+get_word_sentiment(tba[i])
    if trigger:
        average=temp/trigger
    return average
...
def find_centroid(polygon):
    ...
    A,Cx,Cy=0,0,0
    for i in range(l-1):
        A=A+(polygon[i][0]*polygon[i+1][1]-polygon[i+1][0]*polygon[i][1])
    A=0.5*A
    if not A:
        centroid=polygon[0]+(0,)
    else:
        for i in range(l-1):
            Cx=Cx+(polygon[i][0]+polygon[i+1][0])*(polygon[i][0]*polygon[i+1][1]-polygon[i+1][0]*polygon[i][1])
            Cy=Cy+(polygon[i][1]+polygon[i+1][1])*(polygon[i][0]*polygon[i+1][1]-polygon[i+1][0]*polygon[i][1])
        centroid=(Cx/(6*A),Cy/(6*A))+(abs(A),)  
    return centroid
...
def find_closest_state(tweet, state_centers):
    ...
    temp=[(n, geo_distance(loc,s)) for n, s in state_centers.items()]
...
def group_tweets_by_state(tweets):
    ...
    us_centers = {n: find_center(s) for n, s in us_states.items()}
    l=len(tweets)
    tweets_by_state = {find_closest_state(tws, us_centers):[] for tws in tweets}
    for i in range(l):
...
def calculate_average_sentiments(tweets_by_state):
    ...
    for n,s in tweets_by_state.items():
        ave,t=0,0
        l=len(s)
        for i in range(l):
            temp=analyze_tweet_sentiment(s[i])
            if temp or temp==0:
                ave=ave+temp
                t=t+1
        if t:
            averaged_state_sentiments[n]=ave/t
